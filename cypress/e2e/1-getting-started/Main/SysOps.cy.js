...
describe('Tracks page', () => {
    it('7 days free trial', () => {
        ...
        cy.get('.px-6').click()
        cy.wait(3000)
     // cy.visit('https://alnafi.com/tracks/sysops')
    ...
    //cy.wait(3000)
     ...
     describe('CAT PAGE VERIFICATION', () => {
      it('price', () => {
        cy.visit('https://alnafi.com/tracks/sysops')
         //yearly
         cy.get('.space-y-8 > .grid > :nth-child(1)').should('have.text','YearlyRs 250,000 Enroll now ')
      cy.wait(2000)
      //half yearly
      cy.get('.space-y-8 > .grid > :nth-child(2)').should('have.text', 'Half YearlyRs 125,000 Enroll now ')
      cy.wait(2000)
      //quarterly
      cy.get('.space-y-8 > .grid > :nth-child(3)').should('have.text','QuarterlyRs 62,500 Enroll now ')
      cy.wait(2000)
      //monthly
  ...
  describe('Add to Cart', () => {
    it('Add and Go to Cart', () => {
      ...
      cy.get('.form-select').select('English')
      cy.get('.text-left > .btn-primary').click()
      cy.wait(5000)
      cy.visit('https://alnafi.com/cart')
      cy.get('.font-semibold > :nth-child(1)').should('have.text', 'Rs 250000')
      //half yearly
