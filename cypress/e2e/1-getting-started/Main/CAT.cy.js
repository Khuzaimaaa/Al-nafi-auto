  ...
  it('7 days free trial', () => {
   ...
   describe('CAT PAGE VERIFICATION', () => {
    it('price', () => {
      cy.visit('https://alnafi.com/tracks/cat')
       //yearly
       cy.get('.space-y-8 > .grid > :nth-child(1)').should('have.text','YearlyRs 88,250 Enroll now ')
    cy.wait(2000)
    //half yearly
    cy.get('.space-y-8 > .grid > :nth-child(2)').should('have.text', 'Half YearlyRs 44,125 Enroll now ')
    cy.wait(2000)
    //quarterly
    cy.get('.space-y-8 > .grid > :nth-child(3)').should('have.text','QuarterlyRs 22,063 Enroll now ')
    cy.wait(2000)
    //monthly
    cy.get('.space-y-8 > .grid > :nth-child(4)').should('have.text','MonthlyRs 7,500 Enroll now ')
    cy.wait(2000)
...
describe('Add to Cart', () => {
  it('Add and Go to Cart', () => {
    ...
    cy.get('.font-semibold > :nth-child(1)').should('have.text', 'Rs 88250')
    //half yearly
    //cy.visit('https://alnafi.com/tracks/cat')
