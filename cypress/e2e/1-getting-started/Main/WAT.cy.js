...
describe('Tracks page', () => {
    it('7 days free trial', () => {
        ...
        cy.get('.px-6').click()
        cy.wait(3000)
      cy.visit('https://alnafi.com/tracks/wat')
     ...
     describe('CAT PAGE VERIFICATION', () => {
      it('price', () => {
        cy.visit('https://alnafi.com/tracks/wat')
         //yearly
         cy.get('.space-y-8 > .grid > :nth-child(1)').should('have.text','YearlyRs 77,750 Enroll now ')
      cy.wait(2000)
      //half yearly
      cy.get('.space-y-8 > .grid > :nth-child(2)').should('have.text', 'Half YearlyRs 38,875 Enroll now ')
      cy.wait(2000)
      //quarterly
      cy.get('.space-y-8 > .grid > :nth-child(3)').should('have.text','QuarterlyRs 19,438 Enroll now ')
      cy.wait(2000)
      //monthly
      cy.get('.space-y-8 > .grid > :nth-child(4)').should('have.text','MonthlyRs 6,650 Enroll now ')
      cy.wait(2000)
  ...
  describe('Add to Cart', () => {
    it('Add and Go to Cart', () => {
      ...
      cy.get('.font-semibold > :nth-child(1)').should('have.text', 'Rs 77750')
      //half yearly
